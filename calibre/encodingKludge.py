...
def fixStupidEncodings(x, silent=False):
    if type(x) == str:
        try:
            return x.decode(config.SYS_ENCODING)
        except UnicodeDecodeError:
            logger.error(u"Unable to decode value: ".format(repr(x)))
            return None
    elif type(x) == unicode:
        return x
    else:
        logger.error(u"Unknown value passed in, ignoring it: {0} ({1}:{2})".format(str(type(x)), repr(x), repr(type(x))))
        return None
...
def ek(func, *args):
    ...
    if os.name == 'nt':
        ...
        result = func(*[x.encode(config.SYS_ENCODING) if type(x) in (str, unicode) else x for x in args])
